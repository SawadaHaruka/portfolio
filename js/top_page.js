class Top_page {
  constructor() {
    ...
    document.addEventListener("DOMContentLoaded", this.two_top(), false);
  }
  ...
  two_anime() {
    ...
    let ts_w = two_stage.width;
    let ts_h = two_stage.height;

  let element_anime=()=> {
    let tri = two_stage.makePolygon(ts_w / 4, ts_h * 2 / 3, 120, 3);
    tri.noStroke().fill = this.gradation_color();
    ...
    for (let i = 0; i < 12; i++) {
      ...
      let rect = two_stage.makeRoundedRectangle(x + ts_w / 2, y + ts_h / 2, 30, 30, 2);
      rect.noStroke().fill = this.gradation_color();
      rect.rotation += rad;
      rects.push(rect);
    }
    let group = two_stage.makeGroup(rects);
    group.translation.set(ts_w * 3 / 4, ts_h / 3);
    ...
    for (let j = 0; j < 12; j++) {
      ...
      let rect2 = two_stage.makeRoundedRectangle(x2 + ts_w / 2, y2 + ts_h / 2, 80, 10, 2);
      rect2.noStroke().fill = '#FF00E0';
      rect2.rotation += rad2;
      rects2.push(rect2);
    }
    let group2 = two_stage.makeGroup(rects2);
    group2.translation.set(ts_w * 4 / 5, ts_h * 2 / 5);
    ...
    let move=()=> {
      ...
      two_stage.bind('update', () => {
        ...
        // group.rotation += (Math.PI * kaiten - group.rotation) * 0.03;
        group.rotation += 0.02;
        ...
        for (let h = 0; h < 12; h++) {
          ...
          if (state == 0) {
            ...
            createjs.Tween.get(group2).to({ fill: '#ffe000' }, 10);//グループ全体に対して
  
            createjs.Tween.get(bbb.translation).to({ x: xxx, y: yyy }, 200);
            createjs.Tween.get(group).to({ fill: '#00e1ff' }, 10);
            ...
            createjs.Tween.get(group2).to({ fill: '#ff0043' }, 10);
    ...
    */
    window.addEventListener("resize", () => {
      ts_w = two_stage.width = window.innerWidth;
      ts_h = two_stage.height = window.innerHeight;

      group.translation.set(ts_w * 3 / 4, ts_h / 3);
      group2.translation.set(ts_w * 4 / 5, ts_h * 2 / 5);

      tri.translation.set(ts_w / 4, ts_h * 2 / 3);
    }, false);
  }
